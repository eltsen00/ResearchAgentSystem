...
```powershell
# Windows PowerShell — 使用 DashScope（通义千问）
$env:LLM_API_KEY = "你的DashScope API Key"
$env:LLM_BASE_URL = "https://dashscope.aliyuncs.com/compatible-mode/v1"
$env:LLM_MODEL_NAME = "qwen-plus"           # 主模型
$env:LLM_LIGHT_MODEL_NAME = "qwen-turbo"    # 轻量模型（摘要/评分）
...
选项:
  -t, --topic TOPIC     研究主题（中英文均可）。不指定则进入交互式输入模式
  --clear-memory        启动前清空长期记忆（ChromaDB），从零开始研究
  --max-rounds N        最大反思轮次（覆盖 config.py，范围 1-10）
  -q, --quiet           静默模式，减少控制台输出
  -v, --verbose         详细输出模式（默认行为）
  -h, --help            查看完整帮助信息
```
...
# 基本用法：指定研究主题
python main.py -t "人工智能在教育领域的应用"

# 完整参数：指定主题 + 清空记忆 + 限制反思轮次
python main.py --topic "量子计算的最新进展" --clear-memory --max-rounds 3

# 静默模式（适合后台运行或脚本调用）
python main.py -t "新能源汽车市场趋势" -q
```
